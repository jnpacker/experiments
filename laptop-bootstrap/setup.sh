...
##############################################################################
# Phase 3: OCM Lab Dashboard (Helm chart from GitHub)
##############################################################################
phase_dashboard() {
    log "Phase 3: Setting up OCM Dashboard on hub"
    ensure_kubeconfig
...
phase_status() {
    ...
    if [[ -f "${HUB_KUBECONFIG}" ]]; then
        ...
        for ns in cert-manager ingress-nginx assisted-installer capi-system capoa-bootstrap-system capoa-controlplane-system "${DASHBOARD_NS}"; do
...
main() {
    ...
    case "${mode}" in
        ...
        all|"")
            ...
            ;;
        *)
            echo "Usage: $0 [--clusters|--dashboard|--ui|--status|--teardown]"
            exit 1
            ;;
    esac
}
